...
The interval $m\in[-1,-0.1]$ is split into nine segments of width $0.1$. For each segment, an approximate solution — again a Chebyshev polynomial in $m$ with cosine-Fourier coefficients — is stored in a file `U_fft_grid_m_<mi0>_<mi>.jld2` (for instance `U_fft_grid_m_-0.1_-0.2.jld2`). The end-point data `U_end.jld2` and `m_end.jld2` are inherited from the local branch.
...
- `main_branch/main_existence_geometry.jl` — global branch and its geometric transitions.
